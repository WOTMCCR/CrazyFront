<template>
  <view class="flex-col section_3" :style="{ background: formData.color }">
    ...
    <view>
      <span class="self-start font title">主题</span>
      <uni-easyinput v-model="formData.title" placeholder="请输入主题"></uni-easyinput>
...
export default {
  ...
  data() {
    return {
      ...
      formData: {
        title: "研讨会",
        address: "培训室1",
        content: "最新框架的实际编码新建的arrangement",
  ...
  methods: {
    submit() {
      uni.$emit('updateArrangementList',this.formData);
      this.handleBack();
    },
    // 返回上一页
    handleBack() {
