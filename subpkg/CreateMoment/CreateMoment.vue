<template>
  <view class="CreateMoment" :style="{ background: formData.color }">
    ...
    <view class="ColorList" >
      <view class="ColorItem" v-for="(item, index) in gradientColors" :key="index" :style="{ background : item }" @click="changeColor(item)"></view>
    </view>
  ...
  data() {
    return {
      ...
      gradientColors: [
        ...
        'linear-gradient(180deg, #D0F7FB 0%, #F7E0EB 100%)',
      ],
...
.button {
  ...
  background-color: #3fb55e; /* Blue background color */
  border-radius: 7.03rpx;
  filter: drop-shadow(0rpx 8.78rpx 13.17rpx #252c6126, 0rpx 3.51rpx 3.51rpx #8890c233);
  overflow: hidden;
  width: 120rpx;
...
.word {
  ...
  font-size: 28.1rpx;
  font-weight: 700;
  line-height: 65rpx; /* Set line height equal to the button height */
