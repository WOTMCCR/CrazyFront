...
export default {
  ...
  data() {
    return {
      ...
      formData: {
        time: "2023-11-30T16:00:00",
        address: "城市大会中心，市城",
        content: "讨论一些东西",
        title: "研讨会",
        color: "linear-gradient(180deg, #F7E0EB 0%, #FED5AD 100%)",
        contact: "workshops@techsummit.com",
        arrangement: [
          {
            title: "研讨会",
            address: "培训室1",
  ...
  onLoad() {
    uni.$on('updateArrangementList', (arrangement) => {
      this.formData.arrangement.push(arrangement);
      console.log(this.formData.arrangement)
    });
  ...
  methods: {
    addnew() {
      uni.navigateTo({
        url: '/subpkg/CreateArrangement/CreateArrangement'
      });
    },
    submit() {

      uni.$emit('updateScheduleList',this.formData);
      this.handleBack();
    },
    // 返回上一页
    handleBack() {
